  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      ...
      <motion.button
        ...
        className={`group flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/[0.03] border border-white/[0.06] hover:border-white/10 hover:bg-white/[0.05] transition-all ${postsPercentage >= 80 ? 'cursor-pointer' : 'cursor-default'}`}
      >
        <span className="text-[10px] uppercase tracking-wider text-white/40">
          {t('dashboard.stats.totalPosts') || 'Posts'}
        </span>
        <div className="flex items-center gap-1">
          <span className="font-mono text-sm font-semibold text-white/90">{subscription.postsThisMonth}</span>
          <span className="font-mono text-[10px] text-white/30">/{currentLimits.maxPostsPerMonth}</span>
        </div>
      ...
      <motion.button
        ...
        className={`group flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/[0.03] border border-white/[0.06] hover:border-white/10 hover:bg-white/[0.05] transition-all ${brandsPercentage >= 80 ? 'cursor-pointer' : 'cursor-default'}`}
      >
        <span className="text-[10px] uppercase tracking-wider text-white/40">Brands</span>
        <div className="flex items-center gap-1">
          <span className="font-mono text-sm font-semibold text-white/90">{brands.length}</span>
          <span className="font-mono text-[10px] text-white/30">/{currentLimits.maxBrands}</span>
        </div>
        <div className="w-8 h-1 bg-white/10 rounded-full overflow-hidden">
          <motion.div
            initial={{ width: 0 }}
      ...
      >
        <span className="text-[10px] uppercase tracking-wider text-white/40">
          {t('dashboard.stats.scheduled') || 'Queued'}
        </span>
        <span className="font-mono text-sm font-semibold text-white/90">{stats.postsScheduled}</span>
